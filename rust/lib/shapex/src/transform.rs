...
pub trait Transformable {
  ...
  fn rotate_about_axis(&mut self, axis: &Axis, angle: Deg<f64>) {
    self.transform(&axis.rotation(angle));
  }
}
...
impl Axis {
  ...
  pub fn rotation(&self, angle: Deg<f64>) -> Matrix4 {
    let translation = Matrix4::from_translation(self.origin.to_vec());
    let rotation = Matrix4::from_axis_angle(self.direction, angle);
...
impl Plane {
  ...
  pub fn from_points(points: &Vec<Point3>) -> Result<Self, PlaneError> {
    // Find suitable points to build a matching plane
  ...
  pub fn from_curves(elems: &Vec<CurveType>) -> Result<Self, PlaneError> {
    ...
    let plane = Self::from_points(&points)?;
  ...
  pub fn unsample(&self, p: Point3) -> Point2 {
    let p_local = self.as_transform().invert().unwrap().transform_point(p);
    Point2::new(p_local.x, p_local.y)
  }
...
fn transform_from_location_and_normal(origin: Point3, mut normal: Vec3) -> Matrix4 {
